...
<template>
  <motion.article
    class="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm ring-1 ring-transparent transition-shadow duration-300 hover:shadow-xl hover:ring-sky-100"
    v-bind="hoverLiftY"
  >
    <!-- Media -->
    <div class="relative aspect-[16/10] w-full shrink-0 overflow-hidden bg-neutral-100">
      ...
      <span
        v-if="tag"
        class="absolute top-3 left-3 rounded-full bg-white/90 px-2.5 py-1 text-xs font-medium text-sky-700 backdrop-blur-sm"
      >
        {{ tag }}
      </span>
    </div>
    ...
    <div class="flex flex-1 flex-col p-5">
      <h3 class="text-base font-semibold text-neutral-900 transition-colors group-hover:text-sky-700">
        {{ title }}
      </h3>
      <p
        v-if="description"
        class="mt-1.5 flex-1 text-sm leading-relaxed text-neutral-500 line-clamp-2"
      >
        {{ description }}
      </p>
      ...
      <div
        class="mt-4 flex items-center gap-1 text-sm font-medium text-neutral-400 transition-colors group-hover:text-sky-700"
      >
