...
<template>
  <div>
    <div class="mb-4 flex items-center justify-between gap-3">
      ...
      <div
        class="rounded-full border border-sky-100 bg-sky-50 px-3 py-1.5 text-xs font-medium text-sky-700"
      >
    ...
    <Modal :open="deleteModalOpen" @close="deleteModalOpen = false">
      <div class="space-y-5">
        <div>
          ...
          <h2 class="mt-2 text-2xl font-bold text-slate-900">Delete this message?</h2>
        </div>

        <div v-if="selectedMessage" class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <p class="text-sm font-semibold text-slate-800">{{ selectedMessage.name }}</p>
          <p class="mt-1 text-sm text-slate-600">{{ selectedMessage.email }}</p>
